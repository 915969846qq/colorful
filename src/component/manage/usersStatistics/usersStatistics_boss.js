...
getuserstatics=(e)=>{
  ...
            }).then((data)=>{
    ...
    Object.keys(data.type).forEach(function(key){
      ...
      Object.keys(data.type).forEach(function(key){  
ageper.push({item:key,count:data.type[key],percent:(data.type[key]/m)})
      })
...
function Basic (){
 ...
 // =====================================商户增加数================================
    ...
    return (
      <div >
        <h1 style={{fontSize:"24px"}}> 商户统计</h1>
       
       <div style={{display:"flex",flexWrap:"wrap",paddingLeft:150}} >
       <div style={{width:500,marginRight:200}}>
          <h2  style={{textAlign:"center",fontSize:20}}>商户种类统计</h2>
          <Labelline />
          </div>
          <div >
               <h2  style={{textAlign:"center",marginBottom:50,fontSize:20}}>商户人数统计</h2>
               <Addnum  />
        </div>
          <div style={{marginTop:"100px"}}>
          <h2 style={{textAlign:"center",fontSize:20}}>商户地区统计</h2>
        <Basic/>
        </div>
