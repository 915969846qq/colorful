    ...
    let coupon = this.props.coupon.map((item, index) => {
      return (
        <tr key={item.faildate}>
          ...
          <td>{item.satisfy}元</td>
          <td>
            {new Date(parseInt(item.faildate))
              .toLocaleString()
              .replace(/:\d{1,2}$/, ' ')}
          </td>
          <td className="operation">
            {this.state.sumprice < item.satisfy ? (
              <p disabled>不可使用</p>
            ) : this.state.flag === 0 ? (
              <p onClick={this.use.bind(this, index)}>使用</p>
            ) : this.state.index === index ? (
              <p onClick={this.notuse.bind(this)}>取消使用</p>
            ) : (
              <p onClick={this.use.bind(this, index)}>使用</p>
            )}
          </td>
        ...
        <div key={index}>
          <label>
            <input
              type="radio"
              name="bank"
              onClick={this.choosebank.bind(this, index)}
            />
        ...
        <div className="ordert-content-content-receiving">
          ...
          <div className="ordert-content-content-receiving-content">
            ...
            <div>
              ...
              <input
                id="name"
                disabled={this.state.localflag === 0 ? true : false}
            ...
            <div>
              ...
              <input
                disabled={this.state.localflag === 0 ? true : false}
                defaultValue={this.state.consigneeinfo.address}
              ></input>
            </div>
            <div>
              <label>手机号码：</label>
              <input
                disabled={this.state.localflag === 0 ? true : false}
                defaultValue={this.state.consigneeinfo.tel}
              ></input>
              <label>固定电话：</label>
              <input
                disabled={this.state.localflag === 0 ? true : false}
                defaultValue={this.state.consigneeinfo.Landline}
              ></input>
            </div>
            ...
            <div>
              ...
              <div className="ordert-content-content-payment-content-footer">
                <div className="payway">
                  ...
                  <div>
                    <label>
                      <input type="radio" id="web" name="pay" value="web" />
                      网上支付
                    </label>
          ...
          <div className="ordert-content-content-goods-content">
            ...
            <div className="ordert-content-content-goods-footer">
              ...
              <div className="ordert-content-content-goods-footer-right">
                ...
                <div>
                  <p>
                    {this.props.carttoorder.length}件商品，商品总金额：￥
                    {this.state.sumprice}
                  </p>
                  <p>运费：￥{this.state.freight}元</p>
                  {this.state.sumprice > this.state.satisfy ? (
                    <p>优惠：￥{this.state.discount}元</p>
                  ) : (
                    <p>优惠：￥&nbsp;元</p>
                  )}
                </div>
...
export default connect(
  MapStateToProps,
  MapDispatchToProps
)(Personal_My_shopping_order_two)
