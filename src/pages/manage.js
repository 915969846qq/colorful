    ...
    console.log(menulist)
      let MenuList=menulist.map((item)=>{
        ...
        if (item.menuChilds.length===0) {//没有子菜单
          return <Menu.Item key={item.componentPath} onClick={this.go} icon={<DesktopOutlined/>}>
            {item.menuName}
          </Menu.Item>
        }else {
          return <SubMenu key={item.componentPath} icon={<UserOutlined />} title={item.menuName}>
